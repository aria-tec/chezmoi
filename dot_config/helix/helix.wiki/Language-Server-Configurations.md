...
Language server for Bash, written in Rust.
...
Note that `bashls` has external dependencies for certain features!
...
Update your `languages.toml` file:
```toml
...
To install the formatter, go to [drools-formatter](https://github.com/iByteABit256/drools-formatter) and either install the latest `drools-fmt` from release assets, or clone the repository and follow the instructions to build it from source, then add the binary to your PATH.

To install the LSP, go to [drools-lsp-helix](https://github.com/iByteABit256/drools-lsp-helix) and follow the installation instructions.

## Eiffel

https://github.com/alschena/eiffel-tools.git
...
Alternatively, using [`asdf`](https://asdf-vm.com/):

```bash
asdf plugin add elixir-ls
asdf install elixir-ls latest
asdf set --home elixir-ls latest
```
...
Example configuration in `languages.toml`
```toml
[[language]]
name = "groovy"
scope = "source.groovy"
file-types = ["groovy", "gradle", "Jenkinsfile"]
language-servers = ["groovyls"]

[language-server.groovyls]
command = "java"
args = ["-jar", "path_to_groovy-language-server-all.jar"]
...
[Made by one of the maintainers of LuaLS](https://github.com/LuaLS/lua-language-server/issues/3017)
...
The default language server is [Marksman](https://github.com/artempyanykh/marksman), binaries are available from [here](https://github.com/artempyanykh/marksman/releases)
...
`ltex-ls-plus` provides grammar and spell checking in markup documents including BibTEX, ConTEXt, Git commit messages, LATEX, Markdown, MDX, Typst, AsciiDoc, Org, Neorg, Quarto, reStructuredText, R Markdown, R Sweave, XHTML. https://github.com/ltex-plus/ltex-ls-plus
...
Additional configuration settings can be added, for example:

- Lower the log level to prevent spamming Helix's log
- Change the diagnostic level to `warning` so errors are not all anxiety inducing red
- Disable the profanity rules and add the word `builtin` to two dictionaries:
...
Currently,
[the ability to add to your user dictionary while running Helix is not supported](https://github.com/ltex-plus/ltex-ls-plus/issues/95),
...
Choose from two language servers: [`nil`](https://github.com/oxalica/nil) or [`nixd`](https://github.com/nix-community/nixd)

This program is available in [NixOS/nixpkgs](https://github.com/NixOS/nixpkgs)
under attribute `nil` (or `nixd`), and is regularly updated.
...
The formatter `nixfmt` is not included and can be installed with
...
OXVG is a language server for linting and formatting SVG. Follow the [installation guide](https://github.com/noahbald/oxvg#building) and [linting setup](https://github.com/noahbald/oxvg/wiki/Linter).
...
Oxlint is a high-performance linter for JavaScript and TypeScript built on the Oxc compiler stack.

Since [Add oxlint language server](https://github.com/helix-editor/helix/pull/14981) it works out of the box by adding `oxlint-language-server` to your config.
...
```toml
[language-server.biome.config.biome]
experimental.rename = true
...
3. [Install ESLint](https://eslint.org/docs/latest/use/getting-started)
